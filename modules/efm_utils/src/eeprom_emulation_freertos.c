...
void EE_Write(EE_Variable_TypeDef *var, uint16_t writeData)
{
  ...
  //FreeRTOS call to enter critical section (disables interrupts of configMAX_SYSCALL_INTERRUPT_PRIORITY or less)
  portENTER_CRITICAL();
  ...
  // FreeRTOS call to exit critical section (reenables FreeRTOS interrupts)
  portEXIT_CRITICAL();
}
