...
void GPIOINT_Init(void)
{
  ...
  NVIC_SetPriority(GPIO_ODD_IRQn, configMAX_SYSCALL_INTERRUPT_PRIORITY); /* Set priority to use with FreeRTOS */

  NVIC_ClearPendingIRQ(GPIO_EVEN_IRQn);
  NVIC_EnableIRQ(GPIO_EVEN_IRQn);
  NVIC_SetPriority(GPIO_EVEN_IRQn, configMAX_SYSCALL_INTERRUPT_PRIORITY+1); /* Set priority to use with FreeRTOS */
}
